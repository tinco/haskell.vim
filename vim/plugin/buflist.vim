...
"	<S-F3> to decrease buffer list width
"	<S-F4> to increase buffer list width
...
function! BufList_Update()
	...
	if(s:buflist_help == 0)
		...
		call append(10, "\" <S-F3> decrease width")
		call append(11, "\" <S-F4> increase width")
		call append(12, "\" <CR> edit file in <F4>")
...
function! BufList_Decrease()
	if s:buflist_columns > 5
		"let s:buflist_columns = s:buflist_columns - 1
		"execute "vertical resize " . s:buflist_columns
	endif
endfunction

function! BufList_Increase()
...
function! s:BufList_CreatePanel()
	...
	let s:buflist_cols_init = 30
	let s:buflist_columns = s:buflist_cols_init
	...
	"nmap <silent> <buffer> <S-F3> :call BufList_Decrease()<CR>
	"nmap <silent> <buffer> <S-F4> :call BufList_Increase()<CR>
